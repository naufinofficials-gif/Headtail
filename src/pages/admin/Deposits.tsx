...
import { ArrowDownCircle, CheckCircle, XCircle, Clock, AlertCircle } from 'lucide-react';
...
export default function AdminDeposits() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-white mb-8">Persetujuan Deposit</h1>
      {/* Filter */}
      ...
      {filteredDeposits.length === 0 ? (
        <div className="p-8 bg-white/5 border border-white/10 rounded-2xl text-center text-slate-400">
          ...
          <p>Tidak ada deposit {filter !== 'all' ? filter : ''}</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-4">
                <div className="p-3 bg-white/5 rounded-lg">
                  ...
                  <p className="text-lg font-bold text-green-400">Rp {trx.amount.toLocaleString('id-ID')}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-lg">
                  <p className="text-sm text-slate-400">Akun Pengirim</p>
